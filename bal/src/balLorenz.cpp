...
int balLorenz::RHS (realtype t, N_Vector x, N_Vector xdot, void * data) {
  ...
  realtype x1, x2, x3;
	// the extended system and the Jacobian matrix
	realtype Y[n][n], J[n][n], F[n][n];
	// the parameters
  realtype sigma, rho, beta;
	balParameters *parameters;
	// counters
  ...
  J[2][2] = -beta;
	...
	// vector field of the first 3 components
  Ith (xdot, 0) = sigma*(x2-x1);
  Ith (xdot, 1) = -x1*x3 + rho*x1 - x2;
  Ith (xdot, 2) = x1*x2 - beta*x3;
...
/*
int balLorenz::Events (realtype t, N_Vector x, realtype * event, void * data) {
	RHS(t,x,xderiv,data);
...
void balLorenz::EventsConstraints (realtype t, N_Vector x, int * constraints, void * data) {
  realtype x1, x2, x3;
  realtype sigma, rho, beta;
  ...
  beta = parameters->At(2);
	...
	RHS(t,x,xderiv,data);
	for(int i=0; i<GetNumberOfEvents(); i++) xdot[i] = Ith(xderiv,i);

  ris[0] = sigma*(xdot[1]-xdot[0]);
  ris[1] = -xdot[0]*x3 - x1*xdot[2] + rho*xdot[0] - xdot[1];;
  ris[2] = xdot[0]*x2 + x1*xdot[1] - beta*xdot[2];
