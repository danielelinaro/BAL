...
/** 
 * Default cycle detection tolerance: this value is one order 
...
class balODESolver : public balObject {
		...
		bool SolveIntersections();
		bool SolveTrajectoryAndIntersections();
		void SetSolutionLength(int length);
		static realtype EuclideanDistanceFromOrigin(N_Vector x, int length);
		static realtype EuclideanDistance(N_Vector x, N_Vector y, int length);
